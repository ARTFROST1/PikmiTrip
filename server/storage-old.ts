import { users, tours, bookings, reviews, favorites, type User, type InsertUser, type Tour, type InsertTour, type Booking, type InsertBooking, type Review, type InsertReview, type Favorite, type InsertFavorite } from "@shared/schema";

export interface IStorage {
  // Users (for multiple auth providers)
  ...
  addFavorite(userId: number, tourId: number): Promise<Favorite>;
  removeFavorite(userId: number, tourId: number): Promise<boolean>;
  isFavorite(userId: number, tourId: number): Promise<boolean>;
}
